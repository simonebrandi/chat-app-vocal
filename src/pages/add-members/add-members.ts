...
export class AddMembersPage {
  ...
  constructor(public navCtrl: NavController, public navParams: NavParams, public dataProvider: DataProvider,
    public loadingProvider: LoadingProvider, public angularfire: AngularFireDatabase, public alertCtrl: AlertController,
        ...
        this.dataProvider.getCurrentUser().snapshotChanges().subscribe((account) => {
          ...
          if (account.payload.val().friends) {
            for (var i = 0; i < account.payload.val().friends.length; i++) {
              this.dataProvider.getUser(account.payload.val().friends[i]).snapshotChanges().subscribe((friendRes) => {
